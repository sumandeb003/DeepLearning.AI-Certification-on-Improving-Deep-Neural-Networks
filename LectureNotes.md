...
The derivative (slope) at a point $(f(\theta),\theta)$ lying on the above curve: $f(\theta)=\theta^3$ is given by: 
 ...
 - Ratio of height to base of the outer large triangle, *i.e.*, $\frac{f(\theta+\epsilon)-f(\theta-\epsilon)}{2\epsilon}$  = $\frac{(\theta+\epsilon)^3 - (\theta-\epsilon)^3}{2\epsilon}$ = $\frac{(\theta(1+\frac{\epsilon}{\theta}))^3 - (\theta(1-\frac{\epsilon}{\theta}))^3}{2\epsilon}$ = $\frac{\theta^3(1+3\frac{\epsilon}{\theta}-1+3\frac{\epsilon}{\theta})}{\epsilon}$ = $3\theta$
