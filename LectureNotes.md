...
20. When training a neural network, one of the techniques to speed up your training is **normalization**. Normalizing the inputs speeds up the NN training. Normalizing your inputs corresponds to two steps:
  - The first is to subtract the mean $mu$ ($=\frac{1}{n}\Sigmax_i$) from each of the training samples. There will be a separate mean for each feature. Subtract the means to the corresponding features of the training and test samples. This subtraction renders the means of all the features of the samples equal to 0.
